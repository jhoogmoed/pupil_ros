...
from feature_matching import warper
...
class main_matcher:
    ...
    def init_publishers(self):
        self.matcher_object = rospy.Publisher('matcher/object', String, queue_size=2) 			#Segnet object publisher
        self.matcher_warp = rospy.Publisher('matcher/warp', Image, queue_size=2)				#Image of car with gaze point publisher 
        self.matcher_warp_img = rospy.Publisher('matcher/warp_img', Image, queue_size=2)		#Warped image of the car and world camera publisher
        self.matcher_crop = rospy.Publisher('matcher/crop', Image, queue_size=2)				#Cropped image of the world camera for feature matching publisher
        self.matcher_pupil = rospy.Publisher('matcher/pupil', Image, queue_size=2)				#Publisher of world camera with gaze point publisher
        self.matcher_gaze_pupil = rospy.Publisher('matcher/gaze_pupil', Point, queue_size=2)	#Gaze point data publisher
        self.matcher_gaze_accu = rospy.Publisher('matcher/gaze_accu', accu, queue_size=2)		#Accuracy data publisher
    ...
    def spin(self):
        ...
        #After first h calculate new h with feature matching
        if self.start is True:
            ...
            x_offset = 0    
            y_offset = 120
            self.x = int(mh_car[0]/mh_car[2]) + x_offset
            self.y = int(mh_car[1]/mh_car[2]) + y_offset
            ...
            try:
                mh_world, img_warp, frame1 = warper.warp(self.image_start, self.image_world)
                img_out_warp = self.cv_bridge.cv2_to_imgmsg(img_warp, 'bgr8')
                self.matcher_warp_img.publish(img_out_warp)
                frame1 = self.cv_bridge.cv2_to_imgmsg(frame1, 'bgr8')
                self.matcher_crop.publish(frame1)
            ...
            #If marker 1 present, update coordinates for accuracy test
